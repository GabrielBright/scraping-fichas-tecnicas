...
DEFAULT_INPUT_DIR = Path(
    r"C:\Users\gabriel.vinicius\Documents\Vscode\Ficha Tecnicas - Veículos Pesados\DAF"
)
DEFAULT_OUTPUT_XLSX = Path("daf_fichas_tecnicas.xlsx")
...
SECTION_PATTERNS = [
    ...
    (r"\btrem de forca\b", "Trem de Forca"),
    (r"\bmotor\b", "Motor"),
    (r"\btransmiss", "Transmissao"),
    (r"\beixo traseiro\b", "Eixo Traseiro"),
    (r"\bcabine\b", "Cabine"),
    (r"\bchassi\b", "Chassi"),
    (r"\bexterior\b", "Exterior"),
    (r"\binterior\b", "Interior"),
    (r"\btanque de combustivel\b", "Tanque de Combustivel"),
]
...
def should_skip_block(text: str) -> bool:
    ...
    if "imagens meramente ilustrativas" in n:
        return True
    if "daf caminhoes brasil reserva-se" in n:
        return True
    if n.startswith("bra-"):
        return True
    return False
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Extrai dados dos PDFs DAF e salva em XLSX."
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
